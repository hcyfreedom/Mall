...
*/
export  default  function pay(pay_way,orderNum,price,successFun,errFun){
    get('/account/getCharge?channel='+pay_way+"&orderNum="+orderNum+"&price="+price,(res)=>{
        if(res.data.msg == "auth"){
            ...
            pingpp.createPayment(res.data.msg, function(result, err){
                if (result == "success") {
                    ...
                    get("/account/paySucceed?orderNum="+orderNum);
                } else if (result == "fail") {
                    errFun();
